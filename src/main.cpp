...
class MasterTable : public BaseTable {
 public:
  ...
  MasterTable(std::string filename) : BaseTable(filename) {
...
class RasterBase : public BaseTable {
 public:
  ...
  RasterBase(std::string filename) : BaseTable(filename) {
    for(int f=0;f<nfeaturesx;f++){
      GotoPosition(gdbtablx, 16 + f * size_tablx_offsets);
      auto feature_offset = ReadInt32(gdbtablx);
...
class RasterProjection : public BaseTable {
 public:
  RasterProjection(std::string filename) : BaseTable(filename){}
};
...
class RasterData : public BaseTable {
 public:
  ...
  RasterData(std::string filename, const Raster *r) : BaseTable(filename){
    for(int f=0;f<nfeaturesx;f++){
      GotoPosition(gdbtablx, 16 + f * size_tablx_offsets);
      auto feature_offset = ReadInt32(gdbtablx);
